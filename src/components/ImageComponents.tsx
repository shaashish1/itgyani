...
export const ReadMeButton: React.FC<ReadMeButtonProps> = ({
  ...
  const getImagePath = () => {
    if (text === 'readMore') {
      return variant === 'primary' ? IMAGE_PATHS.buttons.readMorePrimary : variant === 'secondary' ? IMAGE_PATHS.buttons.readMoreSecondary : IMAGE_PATHS.buttons.readMoreOutline;
    }
    return IMAGE_PATHS.buttons[text] || IMAGE_PATHS.buttons.readMorePrimary;
  };
  return <button onClick={onClick} className={`inline-block transition-transform hover:scale-105 ${className}`} aria-label={`${text.replace(/([A-Z])/g, ' $1').toLowerCase()} button`}>
      <img src={getImagePath()} alt={text.replace(/([A-Z])/g, ' $1')} className="w-auto h-auto" />
    </button>;
};
...
export const BlogThumbnail: React.FC<BlogThumbnailProps> = ({
  ...
  return <div className={`overflow-hidden rounded-lg ${className}`}>
      <img src={IMAGE_PATHS.blogThumbnails[type]} alt={alt || `${type.replace(/([A-Z])/g, ' $1')} thumbnail`} className="w-full h-full object-cover transition-transform hover:scale-105" />
    </div>;
};
...
export const SocialIcon: React.FC<SocialIconProps> = ({
  ...
  return <Component {...href ? {
    ...
    rel: 'noopener noreferrer'
      ...
      <img src={IMAGE_PATHS.social[type]} alt={`${type} social icon`} className="w-10 h-10" />
    </Component>;
};
  ...
  return <div className="space-y-8 p-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.keys(IMAGE_PATHS.blogThumbnails).map(key => <BlogThumbnail key={key} type={key as keyof typeof IMAGE_PATHS.blogThumbnails} className="aspect-video" />)}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-4">
          {Object.keys(IMAGE_PATHS.pageHeaders).map(key => <PageHeader key={key} type={key as keyof typeof IMAGE_PATHS.pageHeaders} className="rounded-lg overflow-hidden" />)}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-4">
          {Object.keys(IMAGE_PATHS.social).map(key => <SocialIcon key={key} type={key as keyof typeof IMAGE_PATHS.social} />)}
        </div>
      </div>
    </div>;
};
